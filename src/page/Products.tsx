...
import type { ProductType } from "../Features/product/types/productTypes";

// initialProducts: hasil server-fetch (ISR/RSC dari app/(main)/products/page.tsx).
const Products: React.FC<{ initialProducts?: ProductType[] }> = ({ initialProducts }) => {
  const { products, loading, error } = useProducts(initialProducts);
      ...
      <>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6 items-stretch">
          {paginatedProducts.map((item) => (
            <ProductCard
              // UPDATE: Mengubah objectId menjadi id menyesuaikan skema Prisma
              key={item.id}
              item={item}
